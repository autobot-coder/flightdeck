...
function candidates(): { command: string; prefixArgs: string[]; source: CliResolution['source'] }[] {
  ...
  const npmRoots = process.platform === 'win32'
    ? [path.join(process.env.APPDATA ?? '', 'npm', 'node_modules'), path.join(process.env.ProgramFiles ?? '', 'nodejs', 'node_modules')]
    : ['/usr/local/lib/node_modules', '/opt/homebrew/lib/node_modules', path.join(home, '.npm-global', 'lib', 'node_modules')];
...
/**
 * Resolve the CLI Mission Control drives. An explicit `cliPath` (config) or
 * `MISSION_CONTROL_CLI` (env) always wins and is never second-guessed — that is the
...
export async function resolveCli(configured?: string): Promise<CliResolution> {
  const explicit = (configured ?? process.env.MISSION_CONTROL_CLI ?? '').trim();
  const list = explicit
    ? [
...
export function formatPreflight(r: PreflightReport): string {
  ...
  if (!r.node.ok) {
    lines.push(
      `✗ Node ${r.node.version} is too old — Mission Control needs Node ${MIN_NODE_MAJOR}+.`,
  ...
  if (!r.cli.found) {
    lines.push(
      ...
      `  Already installed somewhere unusual? Set "cliPath" in mission.config.json to its full path.`,

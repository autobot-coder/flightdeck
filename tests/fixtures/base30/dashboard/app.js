...
function setupNeeded() {
  ...
  return !s.cli || !s.cli.ready || !s.has_workspaces;
}
...
function renderSetup() {
  ...
  if (cli.ready) {
    ...
    html =
      '<p class="setup-note">Mission Control drives the Claude Code CLI on this machine — that is how ' +
      ...
      'Already installed somewhere unusual? Set <code>"cliPath"</code> in <code>mission.config.json</code>.</p>' +
 ...
 * (Mission Control.dc.html L639-692) on real data; spend intents answer in
...
function normSpeech(s) {
  ...
  // match "DataHub" / "mission-control" workspace names. Matching only, never displayed.

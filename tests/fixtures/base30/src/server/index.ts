...
export async function startServer(
  ...
  const setupJson = () => ({
    ...
    node: report ? { version: report.node.version, ok: report.node.ok } : null,
  ...
  app.post('/api/pick-folder', async (_req, reply) => {
    ...
    try {
      return await new Promise((resolve) => {
        execFile(
          'osascript',
          ['-e', 'tell application "System Events" to activate', '-e', 'POSIX path of (choose folder with prompt "Select a project folder for Mission Control")'],
          { timeout: 300_000 },
          (err, stdout, stderr) => {
            if (err) {
  ...
  app.post('/api/workspaces/:id/pause', async (req) => {
    const { id } = req.params as { id: string };
    store.setRunning(id, false);
    return { running: false };
  });

  app.post('/api/workspaces/:id/resume', async (req) => {
    const { id } = req.params as { id: string };
